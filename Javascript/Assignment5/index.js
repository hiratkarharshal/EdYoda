$(function (){
    $.get('https://5d76bf96515d1a0014085cf9.mockapi.io/quiz',function (res) {
        ...
        for(let i=0; i < res.length ; i++){
            questionWrap += `<div class="Qdiv-wrapper"> <h2 class="quiz-question"> Q${(i+1) + '. '+res[i].question}<h2>`;
        ...
        let submitwrapper=$(`<input type="submit" id='form-submit'> <button type="reset" class="resetBtn" id="resetBtn">Submit</button>`);
        ...
        $("#form-submit").click(function (e) {
            ...
            if(finalApires){
                for (let i = 0; i < getAnswer.length; i++) {
                    ...
                    console.log(options);
                    if(finalApires[getAnswer[i].name - 1]['answer'] == options.indexOf(getAnswer[i].value) + 1){
                        finalScore++;
                    }
                }
